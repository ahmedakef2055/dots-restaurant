<x-layouts.app :title="__('ui.orders.title', ['default' => 'Orders'])">
    <div x-data="{ showDeleteModal: false, deleteAction: '', deleteOrderNumber: '' }">
            ...
            <div>
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <h1 class="text-2xl font-extrabold text-[var(--on-surface)]  tracking-tight">
                        {{ __('ui.orders.title', ['default' => 'Orders']) }}
                    </h1>
                </div>
                <p class="text-sm text-[var(--on-surface-var)] dark:text-[var(--outline)] ml-12">
                    {{ __('ui.orders.subtitle', ['default' => 'Manage and track all restaurant orders']) }}
                </p>
            </div>
        </div>
                ...
                <form method="GET" action="{{ route('orders.index') }}" class="flex flex-wrap items-end gap-3">
                    ...
                    <div class="flex-1 min-w-[160px] max-w-xs">
                        <label class="block text-[10px] font-bold text-[var(--outline)] dark:text-[var(--on-surface-var)] mb-1.5 uppercase tracking-widest">
                            {{ __('ui.orders.filter.search', ['default' => 'Search']) }}
                        </label>
                        <div class="relative">
                            ...
                            <input type="text" name="q" value="{{ $filters['q'] }}"
                                   placeholder="{{ __('ui.orders.filter.search_placeholder', ['default' => 'Order number…']) }}"
                    ...
                    <div class="w-36">
                        <label class="block text-[10px] font-bold text-[var(--outline)] dark:text-[var(--on-surface-var)] mb-1.5 uppercase tracking-widest">
                            {{ __('ui.orders.filter.status', ['default' => 'Status']) }}
                        </label>
                        <select name="status" class="w-full h-9 px-3 bg-[var(--surface-low)]  border border-[var(--outline-var)] dark:border-[var(--outline-var)] rounded-lg text-sm text-[var(--on-surface)]  transition-all outline-none" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor=''">
                            <option value="">{{ __('ui.orders.filter.all_statuses', ['default' => 'All Statuses']) }}</option>
                            <option value="pending" @selected($filters['status']==='pending')>{{ __('ui.orders.status.pending', ['default' => 'Pending']) }}</option>
                            <option value="paid" @selected($filters['status']==='paid')>{{ __('ui.orders.status.paid', ['default' => 'Paid']) }}</option>
                            <option value="cancelled" @selected($filters['status']==='cancelled')>{{ __('ui.orders.status.cancelled', ['default' => 'Cancelled']) }}</option>
                        </select>
                    </div>
                    ...
                    <div class="w-36">
                        <label class="block text-[10px] font-bold text-[var(--outline)] dark:text-[var(--on-surface-var)] mb-1.5 uppercase tracking-widest">
                            {{ __('ui.orders.filter.type', ['default' => 'Type']) }}
                        </label>
                        <select name="order_type" class="w-full h-9 px-3 bg-[var(--surface-low)]  border border-[var(--outline-var)] dark:border-[var(--outline-var)] rounded-lg text-sm text-[var(--on-surface)]  transition-all outline-none" onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor=''">
                            <option value="">{{ __('ui.orders.filter.all_types', ['default' => 'All Types']) }}</option>
                            <option value="dine_in" @selected($filters['order_type']==='dine_in')>{{ __('ui.orders.type.dine_in', ['default' => 'Dine In']) }}</option>
                            <option value="takeaway" @selected($filters['order_type']==='takeaway')>{{ __('ui.orders.type.takeaway', ['default' => 'Takeaway']) }}</option>
                            <option value="delivery" @selected($filters['order_type']==='delivery')>{{ __('ui.orders.type.delivery', ['default' => 'Delivery']) }}</option>
                        </select>
                    </div>
                    ...
                    <div class="w-36">
                        <label class="block text-[10px] font-bold text-[var(--outline)] dark:text-[var(--on-surface-var)] mb-1.5 uppercase tracking-widest">
                            {{ __('ui.orders.filter.from', ['default' => 'From Date']) }}
                        </label>
                    ...
                    <div class="w-36">
                        <label class="block text-[10px] font-bold text-[var(--outline)] dark:text-[var(--on-surface-var)] mb-1.5 uppercase tracking-widest">
                            {{ __('ui.orders.filter.to', ['default' => 'To Date']) }}
                        </label>
                    ...
                    <div class="flex items-center gap-2 ml-auto">
                        ...
                        <button type="submit"
                            ...
                            {{ __('ui.orders.filter.apply', ['default' => 'Apply Filter']) }}
                        </button>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr style="background:linear-gradient(to right,color-mix(in srgb,var(--primary) 5%,transparent),transparent)">
                            ...
                            <th class="px-5 py-3.5 text-[10px] font-extrabold text-[var(--outline)] dark:text-[var(--on-surface-var)] uppercase tracking-widest border-b border-[var(--outline-var)] dark:border-[var(--outline-var)]">
                                {{ __('ui.orders.col.type', ['default' => 'Type']) }}
                            </th>
                            <th class="px-5 py-3.5 text-[10px] font-extrabold text-[var(--outline)] dark:text-[var(--on-surface-var)] uppercase tracking-widest border-b border-[var(--outline-var)] dark:border-[var(--outline-var)]">
                                {{ __('ui.orders.col.table', ['default' => 'Table']) }}
                            </th>
                            <th class="px-5 py-3.5 text-[10px] font-extrabold text-[var(--outline)] dark:text-[var(--on-surface-var)] uppercase tracking-widest border-b border-[var(--outline-var)] dark:border-[var(--outline-var)]">
                                {{ __('ui.orders.col.status', ['default' => 'Status']) }}
                            </th>
                            <th class="px-5 py-3.5 text-[10px] font-extrabold text-[var(--outline)] dark:text-[var(--on-surface-var)] uppercase tracking-widest border-b border-[var(--outline-var)] dark:border-[var(--outline-var)]">
                                {{ __('ui.orders.col.items', ['default' => 'Items']) }}
                            </th>
                            <th class="px-5 py-3.5 text-[10px] font-extrabold text-[var(--outline)] dark:text-[var(--on-surface-var)] uppercase tracking-widest border-b border-[var(--outline-var)] dark:border-[var(--outline-var)]">
                                {{ __('ui.orders.col.total', ['default' => 'Total']) }}
                            </th>
                            <th class="px-5 py-3.5 text-[10px] font-extrabold text-[var(--outline)] dark:text-[var(--on-surface-var)] uppercase tracking-widest border-b border-[var(--outline-var)] dark:border-[var(--outline-var)]">
                                {{ __('ui.orders.col.created', ['default' => 'Date']) }}
                            </th>
                            <th class="px-5 py-3.5 text-[10px] font-extrabold text-[var(--outline)] dark:text-[var(--on-surface-var)] uppercase tracking-widest border-b border-[var(--outline-var)] dark:border-[var(--outline-var)] text-right">
                                {{ __('ui.orders.col.actions', ['default' => 'Actions']) }}
                            </th>
                        </tr>
                    </thead>
                        ...
                        <tr class="group transition-all duration-150" style="cursor:pointer"
                                ...
                                <a href="{{ route('orders.show', $order) }}"
                                   ...
                                   style="" onmouseover="this.style.color='var(--primary)'" onmouseout="this.style.color=''">
                                    ...
                                    <span class="flex flex-col leading-tight">
                                        ...
                                        <span class="text-[10px] font-mono font-normal text-[var(--outline)] dark:text-[var(--on-surface-var)]">{{ $order->order_number }}</span>
                                    </span>
                                </a>
                            </td>
                            ...
                            <td class="px-5 py-4">
                                ...
                                <div class="inline-flex items-center gap-1.5 text-sm text-[var(--on-surface-var)] dark:text-[var(--on-surface-var)]">
                                    <x-icon name="{{ $typeIcon }}" class="text-[15px] text-[var(--outline)]"  />
                                    {{ str($order->order_type)->replace('_', ' ')->title() }}
                                </div>
                            </td>
                            ...
                            <td class="px-5 py-4 text-sm text-[var(--on-surface-var)] dark:text-[var(--outline)]">
                                <div class="flex flex-col">
                                    <span class="font-medium text-[var(--on-surface)] dark:text-[var(--on-surface-var)]">{{ $order->created_at->format('M d, Y') }}</span>
                                    <span class="text-[11px]">{{ $order->created_at->format('g:i A') }}</span>
                                </div>
                            </td>
                            ...
                            <td colspan="7" class="py-20 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-sm font-semibold text-[var(--on-surface-var)] dark:text-[var(--outline)]">
                                        {{ __('ui.orders.empty', ['default' => 'No orders found.']) }}
                                    </p>
        ...
        {{-- ═══ Delete Modal (teleported to body for full-screen blur) ═══ --}}
            ...
            <div x-cloak x-show="showDeleteModal"
                ...
                <div x-show="showDeleteModal"
                    ...
                    <h3 class="text-lg font-bold mb-2" style="color:var(--on-surface)">
                        {{ __('ui.orders.delete_modal.title', ['default' => 'Delete Order']) }}
                    </h3>
                    <p class="text-sm mb-6" style="color:var(--on-surface-var)">
                        {{ __('ui.orders.delete_modal.message_prefix', ['default' => 'Are you sure you want to delete order']) }}
                        <strong x-text="deleteOrderNumber" class="font-mono" style="color:var(--on-surface)"></strong>{{ __('ui.orders.delete_modal.question_mark', ['default' => '?']) }}
                    ...
                    <div class="flex gap-3">
                        <button type="button"
                            ...
                            {{ __('ui.common.cancel', ['default' => 'Cancel']) }}
                        </button>
                        <form :action="deleteAction" method="POST" data-delete-confirm-skip="true" class="flex-1">
                            ...
                            <button type="submit"
                                ...
                                {{ __('ui.common.delete', ['default' => 'Delete']) }}
                            </button>
                        </form>
                    </div>
                </div>
