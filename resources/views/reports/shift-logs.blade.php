<x-layouts.app title="Shift Logs">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('reports.shift-logs.exportPdf', request()->query()) }}" class="glass-button-secondary rounded-xl py-2 px-3 text-sm font-medium flex items-center gap-2">
    ...
    {{-- Filters --}}
    <form method="GET" action="{{ route('reports.shift-logs') }}"
        ...
        <div class="relative flex-1 min-w-52">
            ...
            <div class="relative">
                <select name="user_id" class="w-full glass-input rounded-xl px-4 py-2 text-sm appearance-none pr-8 [&>option]:bg-[var(--surface-lowest)] [&>option]:text-[var(--on-surface)]">
                    ...
                    <option value="{{ $cashier->id }}" @selected((int) $filters['user_id'] === (int) $cashier->id)>{{ $cashier->name }}</option>
                    @endforeach
                </select>
        ...
        <div class="flex items-end gap-2 pb-px">
            ...
            <a href="{{ route('reports.shift-logs') }}" class="glass-button-secondary rounded-xl py-2 px-4 text-sm font-medium">Reset</a>
        </div>
    </form>
    ...
    <div class="glass-panel-elevated rounded-2xl overflow-hidden">
        <div class="px-5 py-4 border-b flex items-center justify-between" style="border-color:color-mix(in srgb,var(--primary) 8%,transparent);background-color:color-mix(in srgb,var(--surface-highest) 20%,transparent)">
            <p class="text-sm font-medium" style="color:var(--on-surface-var)">{{ $shiftLogs->total() }} shift logs</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr style="background-color:color-mix(in srgb,var(--surface-highest) 30%,transparent);border-bottom:1px solid color-mix(in srgb,var(--primary) 8%,transparent)">
                        @foreach(['ID', 'Cashier', 'Shift Start', 'Shift End', 'Duration', 'Cash Diff', 'Actions'] as $h)
                ...
                <tbody class="text-sm divide-y" style="border-color:color-mix(in srgb,var(--primary) 5%,transparent)">
                    ...
                    <tr class="transition-colors"
                        ...
                        <td class="px-5 py-3 font-mono text-xs" style="color:var(--on-surface-var)">#{{ $log->id }}</td>
                        ...
                        <td class="px-5 py-3" style="color:var(--on-surface-var)">{{ $log->shift_start?->format('Y-m-d g:i A') ?? '-' }}</td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)">{{ $log->shift_end?->format('Y-m-d g:i A') ?? '-' }}</td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)">
                            @if($log->shift_start && $log->shift_end)
                                @php $mins = $log->shift_start->diffInMinutes($log->shift_end); @endphp
                        ...
                        <td class="px-5 py-3">
                            @php $diff = $log->cash_difference; @endphp
                            @if($diff !== null)
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold
                                    {{ $diff == 0 ? '' : ($diff > 0 ? 'bg-[color-mix(in_srgb,var(--success)_10%,transparent_90%)] text-[var(--success)] border border-[color-mix(in_srgb,var(--success)_20%,transparent_80%)]' : 'bg-[color-mix(in_srgb,var(--error)_10%,transparent_90%)] text-[var(--error)] border border-[color-mix(in_srgb,var(--error)_20%,transparent_80%)]') }}"
                                    @if($diff == 0) style="background-color:color-mix(in srgb,var(--primary) 10%,transparent);color:var(--primary);border:1px solid color-mix(in srgb,var(--primary) 20%,transparent)" @endif>
                                    {{ $diff >= 0 ? '+' : '' }}{{ \App\Support\CurrencyFormatter::format($diff) }}
                                </span>
                            @else
                        ...
                        <td class="px-5 py-3">
                            @if($canViewShiftLogProfile)
                            <a href="{{ route('reports.shift-logs.profile', $log) }}" class="glass-button-secondary rounded-lg px-3 py-1.5 text-xs font-medium inline-flex items-center gap-1">
        ...
        @if($shiftLogs->hasPages())
        <div class="px-5 py-4 border-t" style="border-color:color-mix(in srgb,var(--primary) 8%,transparent)">{{ $shiftLogs->links() }}</div>
        @endif
    </div>
</x-layouts.app>
