<x-layouts.app :title="__('ui.categories.edit_page_title', ['name' => $category->name])">
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('categories.index') }}"
           class="glass-button-secondary rounded-xl py-2 px-4 text-sm font-medium flex items-center gap-2 shrink-0">
            ...
            {{ __('ui.categories.back') }}
        </a>
        <div class="min-w-0">
            <h1 class="text-3xl font-bold tracking-tight truncate" style="color:var(--on-surface)">{{ __('ui.categories.edit_card_title', ['name' => $category->name]) }}</h1>
            <p class="text-sm mt-0.5" style="color:var(--on-surface-var)">{{ __('ui.categories.edit_subtitle') }}</p>
        </div>
    </div>
    ...
    <div class="glass-panel-elevated rounded-2xl overflow-hidden max-w-2xl">
        <form method="POST" action="{{ route('categories.update', $category) }}">
            @csrf @method('PUT')
            <div class="px-6 py-6">
                @include('categories._form', ['category' => $category, 'mainCategories' => $mainCategories])
            </div>
            <div class="border-t px-6 py-5 flex items-center justify-end gap-3"
                ...
                <a href="{{ route('categories.index') }}" class="glass-button-secondary rounded-xl py-2.5 px-6 text-sm font-medium">{{ __('ui.common.cancel') }}</a>
