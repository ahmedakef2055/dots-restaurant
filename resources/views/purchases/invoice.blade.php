...
<head>
    ...
    <title>{{ app()->getLocale() === 'ar' ? 'فاتورة مشتريات' : 'Purchase Receipt' }} {{ $purchase->purchase_number }}</title>
    <style>
        :root {
            --paper-width: 80mm;
            --brown:  #5E7D67;
...
<body class="{{ (isset($isDirectPrint) && $isDirectPrint) ? 'thermal' : '' }}">
    ...
    <script>
        ...
        function triggerDirectPrint() {
            ...
            btn.disabled = true;
            btn.innerText = '{{ $isArabic ? 'جاري الطباعة...' : 'Printing...' }}';

            fetch('{{ route('purchases.direct-print', $purchase->id) }}', {
                method: 'POST',
                headers: {
            ...
            .then(function(res) {
                if (res.ok) {
                    showToast('{{ $isArabic ? 'تم إرسال الفاتورة للطابعة بنجاح \u2714' : 'Sent to printer successfully \u2714' }}', 'success');
                } else {
    ...
    <main class="receipt">
        <header class="header">
            @if($logoPath)
            <img src="{{ asset($logoPath) }}" alt="{{ config('app.name') }}" class="logo">
            @endif
        ...
        <section class="meta">
            <div class="row">
                <span class="label">{{ $labels['purchase_number'] }}</span>
                <span class="value">{{ $purchase->purchase_number }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['purchase_date'] }}</span>
                <span class="value">{{ $purchase->purchase_date?->format('Y-m-d') ?? $labels['dash'] }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['request_type'] }}</span>
                <span class="value">{{ $requestTypeLabel }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['supplier'] }}</span>
                <span class="value">{{ $purchase->supplier?->name ?? $labels['dash'] }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['processed_by'] }}</span>
                <span class="value">{{ $purchase->user?->name ?? $labels['system'] }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['payment'] }}</span>
                <span class="value">{{ $paymentLabel }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['invoice_number'] }}</span>
                <span class="value">{{ $purchase->invoice_number ?: $labels['dash'] }}</span>
            </div>
        </section>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="num">{{ \App\Support\CurrencyFormatter::format((float) $item->unit_cost) }}</td>
                        <td>{{ $item->expiry_date?->format('Y-m-d') ?? $labels['dash'] }}</td>
                        <td class="num">{{ \App\Support\CurrencyFormatter::format((float) $item->line_total) }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5">{{ $labels['dash'] }}</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <section class="expense-block">
            <p class="section-title">{{ $labels['expense_details'] }}</p>
            <div class="row">
                <span class="label">{{ $labels['expense_title'] }}</span>
                <span class="value">{{ $purchase->expense_title ?: $labels['dash'] }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['expense_reference'] }}</span>
                <span class="value">{{ $purchase->expense_invoice_reference ?: $labels['dash'] }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['expense_amount'] }}</span>
                <span class="value">{{ \App\Support\CurrencyFormatter::format((float) ($purchase->expense_amount ?? $purchase->total)) }}</span>
            </div>
        </section>
        @endif
        ...
        <section class="totals">
            <div class="row">
                ...
                <span class="value">{{ \App\Support\CurrencyFormatter::format((float) $purchase->subtotal) }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['tax'] }}</span>
                <span class="value">{{ \App\Support\CurrencyFormatter::format((float) $purchase->tax_amount) }}</span>
            </div>
            <div class="row">
                <span class="label">{{ $labels['discount'] }}</span>
                <span class="value">{{ \App\Support\CurrencyFormatter::format((float) $purchase->discount_amount) }}</span>
            </div>
            <div class="row grand">
                <span>{{ $labels['total'] }}</span>
                <span>{{ \App\Support\CurrencyFormatter::format((float) $purchase->total) }}</span>
            </div>
        </section>
