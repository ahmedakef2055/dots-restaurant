...
<title>{{ __('ui.financial.exports.title') }}</title>
<style>
@page { margin: 11mm; }
...
<tr>
    ...
    @endif
    <td class="title-cell">
        <div class="rpt-title">{{ __('ui.financial.exports.title') }}</div>
        <div class="rpt-meta">
            {{ __('ui.financial.exports.period') }}: <span dir="ltr" style="unicode-bidi:embed">{{ $from }} → {{ $to }}</span>
            &nbsp;|&nbsp;
            {{ __('ui.financial.exports.generated_at') }}: <span dir="ltr" style="unicode-bidi:embed">{{ $generatedAt?->format('Y-m-d g:i A') }}</span>
        </div>
    </td>
</tr>
</table>
...
<tr>
    <td class="kpi-cell kpi-income">
        <div class="kpi-label">{{ __('ui.financial.summary.total_income') }}</div>
        <div class="kpi-val">{{ $formatter::format($incomeTotal) }}</div>
    </td>
    <td class="kpi-cell kpi-expense">
        <div class="kpi-label">{{ __('ui.financial.summary.total_expenses') }}</div>
        <div class="kpi-val">{{ $formatter::format($expenseTotal) }}</div>
    </td>
    <td class="kpi-cell {{ $net >= 0 ? 'kpi-net-pos' : 'kpi-net-neg' }}">
        <div class="kpi-label">{{ __('ui.financial.summary.net') }}</div>
        <div class="kpi-val">{{ $formatter::format($net) }}</div>
    </td>
    <td class="kpi-cell kpi-period">
        <div class="kpi-label">{{ $isAr ? 'الفترة' : 'Period' }}</div>
        <div class="kpi-val" dir="ltr">{{ $from }} → {{ $to }}</div>
    </td>
    <td class="kpi-cell" style="background:#EEF3EA;border-color:#A8C89A">
        <div class="kpi-label">{{ $isAr ? 'عدد المعاملات' : 'Transactions' }}</div>
        <div class="kpi-val" style="color:#5E7D67">{{ $transactions->count() }}</div>
    </td>
</tr>
</table>
...
<table class="data">
    <thead>
        <tr>
            <th>{{ __('ui.financial.table.type') }}</th>
            <th>{{ __('ui.financial.table.reference') }}</th>
            <th>{{ __('ui.financial.table.description') }}</th>
            <th>{{ __('ui.financial.table.payment_method') }}</th>
            <th>{{ __('ui.financial.table.amount') }}</th>
            <th>{{ __('ui.financial.table.remaining') }}</th>
            <th>{{ __('ui.financial.table.actor') }}</th>
            <th>{{ __('ui.financial.table.status') }}</th>
            <th>{{ __('ui.financial.table.date') }}</th>
        </tr>
    </thead>
    <tbody>
        @forelse($transactions as $row)
        <tr>
            <td><span class="type-badge type-{{ $row['type'] }}">{{ $row['type_label'] }}</span></td>
            <td dir="ltr" style="unicode-bidi:embed">{{ $row['reference'] }}</td>
            <td>{{ $row['description'] }}</td>
            <td><span class="pay-badge pay-{{ $row['payment_method'] }}">{{ $row['payment_method_label'] }}</span></td>
            <td class="{{ $row['category'] === 'income' ? 'amount-income' : 'amount-expense' }}" dir="ltr" style="unicode-bidi:embed">
                {{ $row['category'] === 'income' ? '+' : '−' }}{{ $row['amount'] }}
            </td>
            <td dir="ltr" style="unicode-bidi:embed">{{ $row['remaining'] ?? '—' }}</td>
            <td>{{ $row['actor'] }}</td>
            <td>
            ...
            <td dir="ltr" style="unicode-bidi:embed">{{ $row['date'] }}</td>
        </tr>
        @empty
        <tr>
            <td colspan="9" style="text-align:center;padding:20px;color:#527060;">
                {{ __('ui.financial.table.no_results') }}
            </td>
        </tr>
        @endforelse
    </tbody>
