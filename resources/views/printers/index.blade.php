<x-layouts.app :title="__('ui.printers.title')">

    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold tracking-tight" style="color:var(--on-surface)">{{ __('ui.printers.title') }}</h1>
            <p class="text-sm mt-1" style="color:var(--on-surface-var)">{{ __('ui.printers.subtitle') }}</p>
        </div>
        <a href="{{ route('printers.create') }}" class="glass-button-primary rounded-xl py-2 px-4 text-sm font-medium flex items-center gap-2 self-start">
    ...
    @php $unassigned = array_diff(array_keys($allJobs), $assignedJobs); @endphp
    ...
    <div class="mb-5 rounded-xl border border-[var(--warning-container)] bg-[var(--warning-container)] px-4 py-3 text-sm text-[var(--warning)] flex items-start gap-2   ">
        ...
        <div>
            <span class="font-semibold">{{ __('ui.printers.unassigned_jobs') }}:</span>
            @foreach($unassigned as $job)
                <span class="mx-1 inline-flex items-center rounded-full bg-[var(--warning-container)] px-2 py-0.5 text-xs font-medium text-[var(--warning)]  ">{{ $allJobs[$job]['label'] }}</span>
            @endforeach
        </div>
    </div>
    @endif
    ...
    <div class="glass-panel rounded-2xl flex flex-col items-center justify-center py-20 text-center">
        ...
        <p class="font-medium" style="color:var(--on-surface-var)">{{ __('ui.printers.no_printers') }}</p>
        <a href="{{ route('printers.create') }}" class="glass-button-primary rounded-xl py-2 px-4 text-sm font-medium mt-4 inline-flex items-center gap-2">
            {{ __('ui.printers.add') }}
        </a>
    </div>
    @else
        ...
        <table class="w-full text-sm border-collapse">
            <thead>
                <tr style="background-color:color-mix(in srgb,var(--surface-highest) 30%,transparent);border-bottom:1px solid color-mix(in srgb,var(--primary) 8%,transparent)">
                    ...
                    <th class="px-5 py-3 text-xs font-semibold uppercase tracking-wider text-start" style="color:var(--on-surface-var)">{{ __('ui.common.actions') }}</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="border-b transition hover:bg-[var(--surface-lowest)]" style="border-color:color-mix(in srgb,var(--primary) 5%,transparent)">
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-medium" style="color:var(--on-surface)">{{ $printer->name }}</span>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-xs" style="color:var(--on-surface-var)">{{ $printer->paper_width }}mm</span>
                        </div>
                    </td>
                    <td class="px-5 py-4">
                        <div class="flex flex-wrap gap-1">
                            @forelse($printer->handles ?? [] as $job)
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('printers.edit', $printer) }}" class="glass-button-secondary rounded-lg px-3 py-1.5 text-xs font-medium">
                                تعديل
                            </a>
                        </div>
                    </td>
