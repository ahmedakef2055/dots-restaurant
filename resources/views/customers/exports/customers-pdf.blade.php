...
<title>{{ __('ui.customers.exports.title') }}</title>
<style>
@page { margin: 11mm; }
...
.b-vip  { background: #A8C89A; color: #1A2B21; }
...
<tr>
    ...
    @endif
    <td class="title-cell">
        <div class="rpt-title">{{ __('ui.customers.exports.title') }}</div>
        <div class="rpt-meta">
            {{ $isAr ? 'أُنشئ' : 'Generated' }}: <span dir="ltr" style="unicode-bidi:embed">{{ $generatedAt?->format('Y-m-d g:i A') }}</span>
            &nbsp;|&nbsp;
            {{ $isAr ? 'السجلات' : 'Records' }}: {{ number_format($exportedCount) }} / {{ number_format($totalCount) }}
        </div>
    </td>
</tr>
</table>
...
<div class="filter-bar">
    ...
    &nbsp;&nbsp;
    <strong>{{ $isAr ? 'السجلات المُصدَّرة' : 'Exported' }}:</strong> {{ number_format($exportedCount) }} / {{ number_format($totalCount) }}
</div>

@if($isTruncated)
<div class="notice">
    {{ str_replace([':exported',':total'],[number_format($exportedCount),number_format($totalCount)], __('ui.customers.exports.truncated_notice')) }}
</div>
@endif

<table class="summary">
<tr>
    <td class="sc"><div class="sv">{{ number_format($exportedCount) }}</div><div class="sl">{{ $isAr ? 'إجمالي العملاء' : 'Total Customers' }}</div></td>
    <td class="sc gold"><div class="sv">{{ number_format($vipCount) }}</div><div class="sl">VIP</div></td>
    <td class="sc"><div class="sv">{{ number_format($totalOrders) }}</div><div class="sl">{{ $isAr ? 'إجمالي الطلبات' : 'Total Orders' }}</div></td>
    <td class="sc gold"><div class="sv">{{ \App\Support\CurrencyFormatter::format($totalRevenue) }}</div><div class="sl">{{ $isAr ? 'إجمالي الإيرادات' : 'Total Revenue' }}</div></td>
</tr>
</table>
...
<tr>
    <th style="width:4%">#</th>
    <th style="width:22%">{{ __('ui.customers.table.name') }}</th>
    <th style="width:13%">{{ __('ui.customers.table.phone') }}</th>
    <th style="width:9%">{{ __('ui.customers.table.type') }}</th>
    <th style="width:8%">{{ __('ui.customers.table.orders') }}</th>
    <th style="width:16%">{{ __('ui.customers.table.total_spent') }}</th>
    <th style="width:28%">{{ __('ui.customers.table.last_order') }}</th>
</tr>
</thead>
<tbody>
...
<tr>
    ...
    <td><span class="badge {{ $isVip ? 'b-vip' : 'b-norm' }}">{{ $typeText }}</span></td>
    <td class="num">{{ number_format((int)($row['orders_count'] ?? 0)) }}</td>
    <td class="num">{{ \App\Support\CurrencyFormatter::format((float)($row['total_spent'] ?? 0)) }}</td>
    <td class="num">{{ $lastOrderTxt }}</td>
</tr>
@empty
<tr><td colspan="7" style="text-align:center;padding:14px;color:#4A6352;font-style:italic">{{ __('ui.customers.exports.no_results') }}</td></tr>
@endforelse
</tbody>
</table>
