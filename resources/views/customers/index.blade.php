<x-layouts.app title="Customers">
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('customers.pdf', request()->query()) }}" target="_blank" class="glass-button-secondary rounded-xl py-2 px-3 sm:px-4 text-sm font-medium flex items-center gap-2" title="Export PDF">
            ...
            <a href="{{ route('customers.create') }}" class="glass-button-primary rounded-xl py-2 px-4 text-sm font-medium flex items-center gap-2">
    ...
    <form method="GET" action="{{ route('customers.index') }}"
        ...
        <div class="relative flex-1 min-w-48">
            ...
            <input type="text" name="q" value="{{ $filters['q'] }}" placeholder="Search customers..."
        ...
        @if($filters['q'])
        <a href="{{ route('customers.index') }}" class="glass-button-secondary rounded-xl py-2 px-3 text-sm">
    ...
    <div class="glass-panel-elevated rounded-2xl overflow-hidden">
        <div class="px-5 py-4 border-b" style="border-color:color-mix(in srgb,var(--primary) 8%,transparent);background-color:color-mix(in srgb,var(--surface-highest) 20%,transparent)">
            <p class="text-sm" style="color:var(--on-surface-var)">{{ $customers->total() }} customers total</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr style="background-color:color-mix(in srgb,var(--surface-highest) 30%,transparent);border-bottom:1px solid color-mix(in srgb,var(--primary) 8%,transparent)">
                        @foreach(['Name','Phone','Type','Orders','Total Spent','Last Order','Actions'] as $h)
                ...
                <tbody class="text-sm divide-y" style="border-color:color-mix(in srgb,var(--primary) 5%,transparent)">
                    ...
                    <tr class="transition-colors"
                        ...
                        <td class="px-5 py-3 font-medium" style="color:var(--on-surface)">{{ $customer->full_name }}</td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)">{{ $customer->phone }}</td>
                        <td class="px-5 py-3">
                        ...
                        <td class="px-5 py-3" style="color:var(--on-surface-var)">{{ (int)($customer->orders_count ?? 0) }}</td>
                        <td class="px-5 py-3 font-semibold font-mono" style="color:var(--on-surface)">{{ \App\Support\CurrencyFormatter::format((float)($customer->total_spent ?? 0)) }}</td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)">{{ $customer->last_order_at ? \Illuminate\Support\Carbon::parse($customer->last_order_at)->format('Y-m-d') : '-' }}</td>
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('customers.show', $customer) }}" class="glass-button-secondary rounded-lg px-3 py-1.5 text-xs font-medium">Profile</a>
                                <a href="{{ route('customers.edit', $customer) }}" class="glass-button-secondary rounded-lg px-3 py-1.5 text-xs font-medium">Edit</a>
        ...
        <div class="px-5 py-4 border-t" style="border-color:color-mix(in srgb,var(--primary) 8%,transparent)">{{ $customers->withQueryString()->links() }}</div>
        @endif
    </div>
</x-layouts.app>
