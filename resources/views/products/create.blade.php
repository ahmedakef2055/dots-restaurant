<x-layouts.app :title="__('ui.products.add_title')">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-3xl font-bold tracking-tight" style="color:var(--on-surface)">{{ __('ui.products.create_title') }}</h1>
            <p class="text-sm mt-0.5" style="color:var(--on-surface-var)">{{ __('ui.products.create_subtitle') }}</p>
        </div>
    </div>
    <div class="glass-panel-elevated rounded-2xl overflow-hidden max-w-3xl mx-auto">
        <form method="POST" action="{{ route('products.store') }}">
            @csrf
            <div class="px-6 py-6">@include('products._form', ['categories' => $categories])</div>
