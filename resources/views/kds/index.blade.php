<x-layouts.app :title="$boardTitle">
    <div
        x-data="displayBoardPage({ stages: @js($stageConfigs), initialBoard: @js($initialBoard) }, { fetchUrl: '{{ $fetchEndpoint }}', transitionTemplate: '{{ $transitionTemplate }}', csrf: '{{ csrf_token() }}', pollingMs: {{ (int) $pollingMs }}, transitionSuccessMessage: '{{ __('messages.success.kds_status_updated') }}' })"
        x-init="init()"
        ...
        <div class="kds-page-header">
            <div>
                <h1 class="kds-page-title">{{ $boardTitle }}</h1>
                <p class="kds-page-subtitle">{{ $boardSubtitle }}</p>
            </div>
            ...
            <div class="kds-stats-row">
                <div class="kds-stat-pill">
                    ...
                    <div>
                        ...
                        <span class="kds-stat-label">{{ __('ui.kds.stages.pending') }}</span>
                    </div>
                </div>
                <div class="kds-stat-pill">
                    ...
                    <div>
                        ...
                        <span class="kds-stat-label">{{ __('ui.kds.stages.preparing') }}</span>
                    </div>
                </div>
                <div class="kds-stat-pill">
                    ...
                    <div>
                        ...
                        <span class="kds-stat-label">{{ __('ui.kds.stages.ready') }}</span>
                    </div>
                </div>
            </div>
        </div>
                                    ...
                                    <div class="kds-timer-wrap" :class="getTimerClass(elapsed)">
                                        ...
                                        <span class="kds-timer-badge" x-show="elapsed >= 600" x-cloak>
                                            <template x-if="elapsed >= 1200">
                                                <span>{{ __('ui.kds.timer.late') }}</span>
                                            </template>
                                            <template x-if="elapsed >= 600 && elapsed < 1200">
                                                <span>{{ __('ui.kds.timer.warning') }}</span>
                                            </template>
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="kds-ticket-footer">
                                    ...
                                    <template x-if="stage.key === 'preparing'">
                                        <button type="button"
                                            ...
                                            <span x-text="elapsed >= 1200 ? '{{ __('ui.kds.actions.force_ready') }}' : '{{ __('ui.kds.actions.done') }}'"></span>
                                        </button>
                                    </template>
