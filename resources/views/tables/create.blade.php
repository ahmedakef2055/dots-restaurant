<x-layouts.app :title="__('ui.tables.add_title')">
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-3xl font-bold tracking-tight" style="color:var(--on-surface)">{{ __('ui.tables.create_title') }}</h1>
            <p class="text-sm mt-0.5" style="color:var(--on-surface-var)">{{ __('ui.tables.create_subtitle') }}</p>
        </div>
    </div>
    <div class="glass-panel-elevated rounded-2xl overflow-hidden max-w-2xl mx-auto">
        <form method="POST" action="{{ route('tables.store') }}">
            @csrf
            <div class="px-6 py-6">@include('tables._form')</div>
