...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => __('ui.products.title')]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(__('ui.products.title'))]); ?>
<div x-data="{ showDeleteModal: false, deleteAction: '', deleteProductName: '' }">

    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
            <h1 class="text-3xl font-bold tracking-tight" style="color:var(--on-surface)"><?php echo e(__('ui.products.title')); ?></h1>
            <p class="text-sm mt-1" style="color:var(--on-surface-var)"><?php echo e(__('ui.products.subtitle')); ?></p>
        </div>
        <a href="<?php echo e(route('products.create')); ?>" class="glass-button-primary rounded-xl py-2 px-4 text-sm font-medium flex items-center gap-2">
    ...
    <form method="GET" action="<?php echo e(route('products.index')); ?>"
        ...
        <div class="relative flex-1 min-w-48">
            ...
            <input name="q" value="<?php echo e($filters['q']); ?>" placeholder="<?php echo e(__('ui.products.search_placeholder')); ?>"
        ...
        <div class="relative">
            <select name="category_id" class="glass-input rounded-xl px-4 py-2 text-sm appearance-none pr-8 [&>option]:bg-[var(--surface-lowest)] [&>option]:text-[var(--on-surface)] min-w-44">
                <option value=""><?php echo e(__('ui.products.all_categories')); ?></option>
                ...
                $lbl = $category->type === 'sub' && $category->parent
                     ...
                     : $category->name;
                ?>
                <option value="<?php echo e($category->id); ?>" <?php if($filters['category_id']===(string)$category->id): echo 'selected'; endif; ?>><?php echo e($lbl); ?></option>
        ...
        <div class="relative">
            <select name="preparation_station" class="glass-input rounded-xl px-4 py-2 text-sm appearance-none pr-8 [&>option]:bg-[var(--surface-lowest)] [&>option]:text-[var(--on-surface)]">
                <option value=""><?php echo e(__('ui.products.all_stations')); ?></option>
                <option value="kitchen" <?php if($filters['preparation_station']==='kitchen'): echo 'selected'; endif; ?>><?php echo e(__('ui.products.stations.kitchen')); ?></option>
                <option value="bar"     <?php if($filters['preparation_station']==='bar'): echo 'selected'; endif; ?>><?php echo e(__('ui.products.stations.bar')); ?></option>
            </select>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" fill="currentColor" class="w-[1.25em] h-[1.25em] inline-flex items-center justify-center align-middle shrink-0 absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-[18px]" style="color:var(--on-surface-var)"><path d="M480-360 280-560h400L480-360Z"/></svg>
        </div>
    ...
    <div class="glass-panel-elevated rounded-2xl overflow-hidden">
        <div class="px-5 py-4 border-b" style="border-color:color-mix(in srgb,var(--primary) 8%,transparent);background-color:color-mix(in srgb,var(--surface-highest) 20%,transparent)">
            <p class="text-sm" style="color:var(--on-surface-var)"><?php echo e(__('ui.products.total', ['count' => $products->total()])); ?></p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr style="background-color:color-mix(in srgb,var(--surface-highest) 30%,transparent);border-bottom:1px solid color-mix(in srgb,var(--primary) 8%,transparent)">
                        <?php $__currentLoopData = ['#', __('ui.products.headers.name'), __('ui.products.headers.category'), __('ui.products.headers.station'), __('ui.products.headers.price'), __('ui.products.headers.actions')]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $h): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <tbody class="text-sm divide-y" style="border-color:color-mix(in srgb,var(--primary) 5%,transparent)">
                    ...
                    <tr class="transition-colors"
                        ...
                        <td class="px-5 py-3 text-xs font-mono" style="color:var(--on-surface-var)"><?php echo e($products->firstItem() + $index); ?></td>
                        <td class="px-5 py-3 font-medium" style="color:var(--on-surface)"><?php echo e($product->name); ?></td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)">
                            <?php if($product->category): ?>
                                <?php if($product->category->type === 'sub' && $product->category->parent): ?>
                        ...
                        <td class="px-5 py-3">
                            <?php if($product->preparation_station === 'bar'): ?>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                ...
                                <?php echo e(__('ui.products.stations.bar')); ?>
                            ...
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                ...
                                <?php echo e(__('ui.products.stations.kitchen')); ?>

                            </span>
                            <?php endif; ?>
                        </td>
                        <td class="px-5 py-3 font-semibold font-mono" style="color:var(--primary)">
                            <?php echo e(\App\Support\CurrencyFormatter::format($product->price)); ?>

                        </td>
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-2">
                                <a href="<?php echo e(route('products.edit', $product)); ?>" class="glass-button-secondary rounded-lg px-3 py-1.5 text-xs font-medium"><?php echo e(__('ui.common.edit')); ?></a>
                                <button type="button"
                                        @click="deleteAction = <?php echo \Illuminate\Support\Js::from(route('products.destroy', $product))->toHtml() ?>; deleteProductName = <?php echo \Illuminate\Support\Js::from($product->name)->toHtml() ?>; showDeleteModal = true"
                    ...
                    <tr><td colspan="6" class="px-5 py-10 text-center text-sm" style="color:var(--on-surface-var)"><?php echo e(__('ui.products.no_results')); ?></td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div x-cloak x-show="showDeleteModal"
        ...
        <div x-show="showDeleteModal"
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-lg font-semibold" style="color:var(--on-surface)"><?php echo e(__('ui.products.delete_modal_title')); ?></h3>
            </div>
            <p class="text-sm mb-6" style="color:var(--on-surface-var)">
                <?php echo e(__('ui.products.delete_modal_message_prefix')); ?>

                <span class="font-semibold" style="color:var(--on-surface)" x-text="deleteProductName"></span><?php echo e(__('ui.products.delete_modal_question_mark')); ?>

            </p>
            <form :action="deleteAction" method="POST" class="flex justify-end gap-3">
                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                <button type="button" @click="showDeleteModal = false" class="glass-button-secondary rounded-xl py-2.5 px-6 text-sm font-medium"><?php echo e(__('ui.common.cancel')); ?></button>
...
<?php endif; ?>
<?php /**PATH /var/www/dots-main/resources/views/products/index.blade.php ENDPATH**/ ?>