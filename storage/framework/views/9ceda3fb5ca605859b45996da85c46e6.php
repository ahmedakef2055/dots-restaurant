...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.layouts.app','data' => ['title' => __('ui.users.title')]] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute(__('ui.users.title'))]); ?>
<?php
...
?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold tracking-tight" style="color:var(--on-surface)"><?php echo e(__('ui.users.title')); ?></h1>
            <p class="text-sm mt-1" style="color:var(--on-surface-var)"><?php echo e(__('ui.users.subtitle')); ?></p>
        </div>
        <?php if($canCreateUsers): ?>
        <a href="<?php echo e(route('users.create')); ?>" class="glass-button-primary rounded-xl py-2 px-4 text-sm font-medium flex items-center gap-2">
    ...
    <form method="GET" action="<?php echo e(route('users.index')); ?>"
        ...
        <div class="relative flex-1 min-w-48">
            ...
            <input type="text" name="q" value="<?php echo e($filters['q']); ?>"
                   placeholder="<?php echo e(__('ui.users.search_placeholder')); ?>"
        ...
        <div class="relative">
            <select name="role_id" class="glass-input rounded-xl px-4 py-2 text-sm appearance-none pr-8 [&>option]:bg-[var(--surface-lowest)] [&>option]:text-[var(--on-surface)] min-w-40">
                <option value=""><?php echo e(__('ui.users.all_roles')); ?></option>
                <?php $__currentLoopData = $roles; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $role): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($role->id); ?>" <?php if($filters['role_id'] !== '' && (int)$filters['role_id'] === (int)$role->id): echo 'selected'; endif; ?>><?php echo e($role->name); ?></option>
    ...
    <div class="glass-panel-elevated rounded-2xl overflow-hidden">
        <div class="px-5 py-4 border-b" style="border-color:color-mix(in srgb,var(--primary) 8%,transparent);background-color:color-mix(in srgb,var(--surface-highest) 20%,transparent)">
            <p class="text-sm" style="color:var(--on-surface-var)"><?php echo e(__('ui.users.total', ['count' => $users->total()])); ?></p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr style="background-color:color-mix(in srgb,var(--surface-highest) 30%,transparent);border-bottom:1px solid color-mix(in srgb,var(--primary) 8%,transparent)">
                        <?php $__currentLoopData = [__('ui.users.headers.name'), __('ui.users.headers.username'), __('ui.users.headers.phone'), __('ui.users.headers.email'), __('ui.users.headers.role'), __('ui.users.headers.permissions'), __('ui.users.headers.status'), __('ui.common.actions')]; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $h): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <tbody class="text-sm">
                    ...
                    <tr class="transition-colors" style="border-bottom:1px solid color-mix(in srgb,var(--primary) 5%,transparent)"
                        ...
                        <td class="px-5 py-3 font-medium" style="color:var(--on-surface)"><?php echo e($user->name); ?></td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)"><?php echo e($user->username ?: '-'); ?></td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)"><?php echo e($user->phone ?: '-'); ?></td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)"><?php echo e($user->email); ?></td>
                        <td class="px-5 py-3" style="color:var(--on-surface-var)"><?php echo e($primaryRoleName); ?></td>
                        <td class="px-5 py-3">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium"
                                  style="background-color:color-mix(in srgb,var(--primary) 10%,transparent);color:var(--primary)">
                                <?php echo e($effectivePermissions->count()); ?>
                        ...
                        <td class="px-5 py-3">
                            <?php if($hasIsActiveColumn): ?>
                                ...
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-[color-mix(in_srgb,var(--success)_10%,transparent_90%)] text-[var(--success)] border border-[color-mix(in_srgb,var(--success)_20%,transparent_80%)]">
                                    <span class="w-1.5 h-1.5 rounded-full bg-[var(--success)] animate-pulse"></span><?php echo e(__('ui.users.status_active')); ?>
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium"
                                    ...
                                    <?php echo e(__('ui.users.status_disabled')); ?>
                        ...
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-1.5 flex-wrap">
                                <a href="<?php echo e(route('users.show', $user)); ?>" class="glass-button-secondary rounded-lg px-2.5 py-1.5 text-xs font-medium"><?php echo e(__('ui.users.view')); ?></a>
                                <?php if($canUpdateUsers): ?>
                                <a href="<?php echo e(route('users.edit', $user)); ?>" class="glass-button-secondary rounded-lg px-2.5 py-1.5 text-xs font-medium"><?php echo e(__('ui.common.edit')); ?></a>
                                <?php endif; ?>
                                <?php if($hasIsActiveColumn && $canUpdateUsers): ?>
                                <form method="POST" action="<?php echo e(route('users.toggle-status', $user)); ?>" class="inline-block">
                                    ...
                                    <button type="submit" class="rounded-lg px-2.5 py-1.5 text-xs font-medium transition-all"
                                            ...
                                            onmouseleave="this.style.backgroundColor=''">
                                        <?php echo e((bool)$user->is_active ? __('ui.users.disable') : __('ui.users.enable')); ?>

                                    </button>
                                </form>
                                <?php endif; ?>
                                <?php if($canDeleteUsers): ?>
                                <form method="POST" action="<?php echo e(route('users.destroy', $user)); ?>" class="inline-block">
                    ...
                    <tr><td colspan="8" class="px-5 py-10 text-center text-sm" style="color:var(--on-surface-var)"><?php echo e(__('ui.users.no_results')); ?></td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
        <?php if($users->hasPages()): ?>
        <div class="px-5 py-4 border-t" style="border-color:color-mix(in srgb,var(--primary) 8%,transparent)"><?php echo e($users->withQueryString()->links()); ?></div>
        <?php endif; ?>
    </div>
...
<?php endif; ?>
<?php /**PATH /var/www/dots-main/resources/views/users/index.blade.php ENDPATH**/ ?>